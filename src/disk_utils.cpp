                              ...
                              const uint32_t Lf)
{
    ...
    Timer timer;
    int num_parts = partition_with_ram_budget<T>(base_file, sampling_rate, ram_budget, 2 * R / 3, merged_index_prefix);
...
template <typename T, typename LabelT>
int core_search_disk_index(diskann::Metric &metric, const std::string &index_path_prefix,
                           const std::string &result_output_prefix, const T *query, size_t query_num, size_t query_dim,
                           size_t query_aligned_dim, std::string &gt_file, const uint32_t num_threads,
                           const uint32_t recall_at, const uint32_t beamwidth, const uint32_t num_nodes_to_cache,
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    diskann::aligned_free(const_cast<T *>(query));
    if (warmup != nullptr)
...
template DISKANN_DLLEXPORT int build_merged_vamana_index<uint8_t, uint16_t>(
    ...
    size_t build_pq_bytes, bool use_opq, uint32_t num_threads, bool use_filters, const std::string &label_file,
    const std::string &labels_to_medoids_file, const std::string &universal_label, const uint32_t Lf);
// Template instantiations for core_search_disk_index
template DISKANN_DLLEXPORT int core_search_disk_index<float, uint32_t>(
    diskann::Metric &metric, const std::string &index_path_prefix, const std::string &result_output_prefix,
    const float *query, size_t query_num, size_t query_dim, size_t query_aligned_dim, std::string &gt_file,
...
template DISKANN_DLLEXPORT int core_search_disk_index<uint8_t, uint32_t>(
    diskann::Metric &metric, const std::string &index_path_prefix, const std::string &result_output_prefix,
    const uint8_t *query, size_t query_num, size_t query_dim, size_t query_aligned_dim, std::string &gt_file,
    const uint32_t num_threads, const uint32_t recall_at, const uint32_t beamwidth, const uint32_t num_nodes_to_cache,
    const uint32_t search_io_limit, const std::vector<uint32_t> &Lvec, const float fail_if_recall_below,
    const std::vector<std::string> &query_filters, const bool use_reorder_data);

template DISKANN_DLLEXPORT int core_search_disk_index<int8_t, uint32_t>(
    diskann::Metric &metric, const std::string &index_path_prefix, const std::string &result_output_prefix,
    const int8_t *query, size_t query_num, size_t query_dim, size_t query_aligned_dim, std::string &gt_file,
    const uint32_t num_threads, const uint32_t recall_at, const uint32_t beamwidth, const uint32_t num_nodes_to_cache,
    const uint32_t search_io_limit, const std::vector<uint32_t> &Lvec, const float fail_if_recall_below,
    const std::vector<std::string> &query_filters, const bool use_reorder_data);
}; // namespace diskann
